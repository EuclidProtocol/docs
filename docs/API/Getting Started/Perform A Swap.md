...
The [swap](../../API Reference/REST/Transactions/Swap.md) parameters are the following:
...
### 1. Get all available tokens
 ...
 The first step will be getting all the available tokens and selecting the input and output tokens. This ensures that the tokens involved in the swap are supported by the Euclid Layer. This can be done using the [`All Tokens`](../../API Reference/GQL/Router/All%20Tokens.md) query:
...
Next, we get the chains that have escrows for `token_in` and then select the one to use. This can be done using the [`Token Denoms`](../../API Reference/GQL/Router/Token%20Denoms.md) query:
 :::tip
...
In many cases, multiple routes can be taken to perform the desired swap. In this step, we will fetch these routes and select the one we want to use. This can be done using the [`Get Routes`](../../API Reference/REST/Routes/Get%20Routes.md) query:
```bash
curl -X 'POST' \
...
Now that we have all the parameters required, we can simulate the swap. This can be done using the [`Simulate Swap`](../../API Reference/GQL/Router/Simulate%20Swap.md) query:
 :::tip
...
Token out might exist on multiple chains. We query the available escrows for the selected token and then prompt the user to choose which chain they want to receive the tokens on. This can be done using the [`Escrows`](../../API Reference/GQL/Router/Escrows.md) query:
:::note
