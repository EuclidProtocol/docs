...
The [Add Liquidity](../../API%20Reference/REST/Transactions/Liquidity/Add%20Liquidity.md) parameters are the following:
...
Now that we have chosen the token pair to add liquidity to, we can check the chains that can be used to add liquidity. This can be done using the [All Pools](../../API%20Reference/GQL/VLP/All%20Pools.md) query:
...
For each of the tokens in the pair, we need to get the allowed denoms. This will be used to specify the `pair_info` field for the add liquidity call. The allowed denoms can be fetched using the [Escrow](../../API%20Reference/GQL/Factory/Escrow.md) query:
