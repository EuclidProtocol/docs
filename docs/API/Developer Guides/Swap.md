...
# Swap
To perform a swap, a swap transaction needs to be generated by calling the `Swap` endpoint in the Rest API and then broadcast to a chain. The endpoint URL is the following:

```bash
http://api.staging.euclidprotocol.com:0/api/v1/execute/swap
```
