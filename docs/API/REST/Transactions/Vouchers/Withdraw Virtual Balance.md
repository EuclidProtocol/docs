...
| `cross_chain_addresses`   | [`CrossChainUserWithLimit`](../../../common%20types.md#crosschainaddresswithlimit)`[]` |  Optional set of addresses to specify where the tokens should be released. The first element specified in the vector has highest priority and so on.  Defaults to sender if not specified.                              |
