...
A compact, read-only price display for a fixed token pair. Shows how much `tokenOut` you'd receive for a given `amountIn`. Refreshes automatically. No wallet required.
...
<BrowserOnly>
  {() => {
    ...
    return <QuoteWidget tokenIn="usdt" tokenOut="eth" amountIn="100" />;
  }}
</BrowserOnly>

## Component Code

Copy this file into your project at `src/components/widgets/QuoteWidget.tsx`:

<details>
<summary>QuoteWidget.tsx (click to expand)</summary>
...
export function QuoteWidget({
  ...
  const formattedOut = bestAmountOut ? fromBaseUnits(bestAmountOut, outMeta?.coinDecimal ?? 6) : null;
...
// Drop anywhere — shows a live USDT → ETH price
<QuoteWidget
  tokenIn="usdt"
  tokenOut="eth"
  amountIn="100"
...
| `refreshIntervalMs` | `number` | `30000` | Poll interval in ms |
