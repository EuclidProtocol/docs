...
A full cross-chain swap UI. The user selects input/output tokens, enters an amount, and the widget fetches the best route and builds a transaction payload ready for your wallet to sign. The sender address and chain are provided by your wallet connection; the widget exposes recipient fields so users can specify where to receive their swapped tokens.
...
Copy this file into your project at `src/components/widgets/SwapWidget.tsx`:

<details>
<summary>SwapWidget.tsx (click to expand)</summary>
      ...
      {loading ? (
        ...
        <>
          ...
          <div className={styles.divider}>
            <button className={styles.swapArrow} onClick={() => { setTokenIn(tokenOut); setTokenOut(tokenIn); setBestPath(null); setUiState("idle"); }}>⇅</button>
          </div>
          ...
          <div className={styles.panel} style={{ position: "relative" }}>
            ...
            <div className={styles.row}>
              <div style={{ flex: 1, fontSize: "1.4rem", fontWeight: 500, color: formattedOut ? "var(--w-text)" : "var(--w-muted)" }}>
                {uiState === "fetching-quote" ? <div className={styles.skeleton} style={{ width: "100px", height: "1.4rem" }} /> : (formattedOut ?? "0.00")}
              </div>
              <div style={{ position: "relative" }}>
          ...
          <div style={{ marginTop: "0.75rem" }}>
            ...
            <select style={{ width: "100%", padding: "0.5rem 0.75rem", borderRadius: "8px", border: "1px solid var(--w-border)", background: "var(--w-panel)", color: "var(--w-text)", fontFamily: "inherit", fontSize: "0.9rem" }}
              value={recipientChainUid} onChange={(e) => setRecipientChainUid(e.target.value)}>
          ...
          {impact !== null && (
            <div className={styles.infoRow}>
              ...
              <span className={`${styles.impactBadge} ${impact > 2 ? styles.impactHigh : ""}`}>{impact.toFixed(2)}%</span>
            </div>
          )}
...
function App() {
  return (
    <SwapWidget
      ...
      onSwapReady={(payload) => {
        // payload is typed SwapPayload
        // EVM: pass payload.msgs[0] to wagmi's sendTransaction
        // Cosmos: pass payload.msgs to CosmJS
        console.log(payload);
      }}
...
|---|---|---|---|
| `senderAddress` | `string` | **required** | Sender wallet address (from your wallet connection) |
| `senderChainUid` | `string` | **required** | Sender's chain UID (from your wallet connection) |
| `defaultTokenIn` | `string` | `"eth"` | Initial input token ID |
| `defaultTokenOut` | `string` | `"usdt"` | Initial output token ID |
| `defaultRecipientChainUid` | `string` | first chain | Default recipient chain UID |
| `slippageBps` | `number` | `500` | Slippage in basis points (500 = 5%) |
| `partnerFeeBps` | `number` | — | Partner fee in basis points |
| `partnerFeeRecipient` | `string` | — | Partner fee recipient address |
| `theme` | `Partial<WidgetTheme>` | — | Custom colors — see [Theming](#theming) |
| `apiBase` | `string` | mainnet URL | Override API base URL |
| `onSwapReady` | `(payload: SwapPayload) => void` | — | Called with the tx payload when ready |
| `onError` | `(error: string) => void` | — | Called on any error |

## Theming

All colors are driven by a `WidgetTheme` object passed as the `theme` prop:
...
<SwapWidget
  ...
  theme={{
    accent: "#6c47ff",              // buttons, active borders
    surface: "#1a1a2e",             // widget background
    panel: "#16213e",               // input panel background
